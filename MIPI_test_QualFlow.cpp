 ...
 * class is created.
 */
class MIPI_test_QualFlow: public testmethod::TestMethod {
		...
		for(double i=Cur_V_value*0.6;i<=Cur_V_value*1.4;i=i+Cur_V_value*(1.4-0.6)/40)
		{
			...
			//  	    	  pll_lock[i] = GET_FUNCTIONAL_RESULT(PLLLock_list[i]);
						  ...
						  if(iDebug)
						  {
							  ...
							  cout  << "Vscale is " << i <<"  Frequency (" << VecVar_list[i] << ") = " << dFrequency[i] << "MHz"<< endl;
						  }
...
REGISTER_TESTMETHOD("bm_ac_tml.MIPI_test_QualFlow", MIPI_test_QualFlow);
