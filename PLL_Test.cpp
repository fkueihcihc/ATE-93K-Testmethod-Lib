 ...
 * class is created.
 */
class PLL_Test: public testmethod::TestMethod {
  ...
  virtual void run()
    {
      ...
      iVSize = VecVar_list.size();
	  iPSize = PLLLock_list.size();

	  //cout<<"iListSize = "<<iListSize<<endl;

	  aiCapData.resize(iVSize);
	  dFrequency.resize(iVSize);
	  pll_lock.resize(iPSize);
	  ...
	  pll_lock.init(0);
  	  ...
  	  ON_FIRST_INVOCATION_END();
  	      ...
  	      for(int i =0;i<iPSize;i++){
  	  		  ...
  	  		  if(iDebug)
  	  		  {
  	  			  cout <<"PLL_LOCK Result = "<<pll_lock[i]<<endl;
  	  		  }
  	      }
...
REGISTER_TESTMETHOD("bm_ac_tml.PLL_Test", PLL_Test);
