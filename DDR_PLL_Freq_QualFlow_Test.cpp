 ...
 * class is created.
 */
class DDR_PLL_Freq_QualFlow_Test: public testmethod::TestMethod {
...
protected:
  ...
  virtual void run()
  {
	...
	int result = 0;
	string test_name;
	for(double i=Cur_V_value*0.6;i<=Cur_V_value*1.4;i=i+Cur_V_value*(1.4-0.6)/40)
	{
		...
		if (debug == "YES")
		{
			...
			cout <<"Vscale is "<< i <<"site:"<<CURRENT_SITE_NUMBER()<< " frequency dto[0] is : " << Calculate_Freq_1 << "  MHz" <<endl;
			cout <<"Vscale is "<< i <<"site:"<<CURRENT_SITE_NUMBER()<< " frequency dto[1] is : " << Calculate_Freq_2 << "  MHz" <<endl;
		}
	}
...
REGISTER_TESTMETHOD("bm_ac_tml.DDR_PLL_Freq_QualFlow_Test", DDR_PLL_Freq_QualFlow_Test);
