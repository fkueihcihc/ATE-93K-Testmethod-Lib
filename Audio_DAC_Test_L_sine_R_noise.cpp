 ...
 * class is created.
 */
class Audio_DAC_Test_L_sine_R_noise: public testmethod::TestMethod {
  ...
  virtual void run()
  {
	...
	for(unsigned int i=0;i<dgtpins_list.size();i++){
        if(i == 0){
		  ...
		  DSP_FFT(dgtz_waveform_R,FFTDataArray,RECT);
          ...
          for(int i =LSL20HzIndex;i<USL20kHzIndex+1;i++){
			...
			if(debug_mode)
			{
				...
				PUT_DEBUG(dgtpins,temp1, dgtz_waveform_R);
				cout << sTsName << " Noise_dBV_R = " << Noise_dBV <<endl;
			}
			TestSet.cont(true).TEST("", "Noise_dBV_R", tmLimits, Noise_dBV);
		    ...
		    DSP_THD(dgtz_waveform_L, &THDArray, RelCycles,3, DB, 0);// calculate THD & SND
  			...
  			if(debug_mode)
  			{
				...
				sprintf(temp1,"orig_waveL_site%d",site_index+1);
  				PUT_DEBUG(dgtpins, temp1, dgtz_waveform_L);
  				cout << sTsName << ":Vout_RMS_L = " << Vout_RMS_L <<endl;
...
REGISTER_TESTMETHOD("bm_analog_tml.Audio_DAC_Test_L_sine_R_noise", Audio_DAC_Test_L_sine_R_noise);
