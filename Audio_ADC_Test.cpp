 ...
 * class is created.
 */
class Audio_ADC_Test: public testmethod::TestMethod {
  ...
  virtual void run()
  {
    ...
    if(rly_on!="None")
	 ...
	 if(rly_off!="None")
		...
		TASK_LIST TASK_3P3;
		   PPMU_SET_FVMI_3P3.pin("AUDIO_VCM").mode("VFIM").vForce(0.9 V).iRange(2 mA).min(-2 mA).max(2 mA);
...
//				                      +(digtal_waveform[i+15] << 15);
	...
	DSP_SPECTRUM(digtal_waveform_L,spect_waveform_L,DB,1.41,RECT,0);
	DSP_SPECTRUM(digtal_waveform_R,spect_waveform_R,DB,1.41,RECT,0);
	...
	if(debug_mode)
	{
		...
		cout << sTsName <<" snr_l :" << snr_l <<endl;
		cout << sTsName <<" snr_r :" << snr_r <<endl;
		cout << sTsName <<" thd_l :" << thd_l <<endl;
		cout << sTsName <<" thd_r :" << thd_r <<endl;
		cout << sTsName <<" sfdr_l :" << sfdr_l <<endl;
		cout << sTsName <<" sfdr_r :" << sfdr_r <<endl;
	}
...
REGISTER_TESTMETHOD("bm_analog_tml.Audio_ADC_Test", Audio_ADC_Test);
